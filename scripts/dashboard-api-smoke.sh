...
unauthenticated_status="$(
  curl -sS -o /dev/null -w "%{http_code}" \
    "${BASE_URL}/api/dashboard/funnels"
)"

if [[ "${unauthenticated_status}" != "401" ]]; then
  echo "Expected unauthenticated /api/dashboard/funnels to return 401, got ${unauthenticated_status}."
  exit 1
fi

echo "PASS: unauthenticated dashboard API returns 401."
